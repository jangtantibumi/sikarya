...
@section('title', 'Currencies - ERP Finance')
@section('page_heading', 'Currency Master (Mata Uang)')
...
<div class="space-y-6">
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="glass-panel p-6 rounded-2xl space-y-4">
            ...
            <form action="{{ route('finance.currencies.store') }}" method="POST" class="space-y-4">
                @csrf
                <div>
                ...
                <tbody class="divide-y divide-slate-800">
                    @forelse($currencies as $curr)
                        <tr class="hover:bg-slate-800/40 transition">
                            <td class="px-4 py-3 font-mono font-bold text-emerald-400">{{ $curr->code }}</td>
                            <td class="px-4 py-3">{{ $curr->name }}</td>
                            <td class="px-4 py-3 font-bold">{{ $curr->symbol }}</td>
                            <td class="px-4 py-3">{{ $curr->decimal_places }}</td>
                            <td class="px-4 py-3">
                                @if($curr->is_base)
                            ...
                            <td class="px-4 py-3">
                                <form action="{{ route('finance.currencies.destroy', $curr->id) }}" method="POST" onsubmit="return confirm('Hapus currency ini?')">
                                    @csrf
                                    @method('DELETE')
