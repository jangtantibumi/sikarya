...
class PurchaseRequestController extends Controller
{
    ...
    public function index()
    {
        ...
        return response()->json(PurchaseRequest::query()->with('lines.product')->latest()->get());
    }
    ...
    {
        $request = PurchaseRequest::query()->findOrFail($id);
        abort_unless(in_array($request->status, ['draft', 'rejected'], true), 422, 'Pengajuan tidak dapat diajukan kembali.');
    ...
    {
        $item = PurchaseRequest::query()->findOrFail($id);
        abort_unless($request->user()->isCEO() || $request->user()->isManager(), 403);
        $data = $request->validate(['status' => 'required|in:approved,rejected']);
        abort_unless($item->status === 'submitted', 422, 'Hanya pengajuan submitted yang dapat diputuskan.');
        $item->update($data);
