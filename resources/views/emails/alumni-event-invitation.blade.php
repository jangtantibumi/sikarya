...
<body style="margin:0;background:#0b0d10;color:#f4f5f7;font-family:Arial,sans-serif;">
    <div style="max-width:620px;margin:0 auto;padding:32px 20px;">
        <div style="background:#15181e;border:1px solid #30343d;border-radius:18px;padding:28px;">
            ...
            <h1 style="font-size:25px;line-height:1.25;margin:14px 0 8px;">{{ $invitation->title }}</h1>
            <p style="color:#b6bbc5;line-height:1.7;margin:0 0 22px;">Halo {{ $alumni->name }},</p>
            <p style="color:#eef0f4;line-height:1.7;white-space:pre-line;">{{ $invitation->message }}</p>
            <div style="margin:24px 0;padding:18px;background:#0f1115;border-radius:12px;border-left:3px solid #f2c94c;">
                <div style="margin-bottom:8px;"><strong>Waktu:</strong> {{ $invitation->event_at?->timezone(config('app.timezone'))->format('d M Y, H:i') }} WIB</div>
                @if($invitation->location)
                    <div><strong>Lokasi:</strong> {{ $invitation->location }}</div>
                @endif
            </div>
            @if($invitation->registration_url)
                <a href="{{ $invitation->registration_url }}" style="display:inline-block;background:#f2c94c;color:#111318;text-decoration:none;font-weight:700;padding:12px 18px;border-radius:10px;">Konfirmasi Kehadiran</a>
            @endif
