...
<div class="max-w-4xl mx-auto">
    ...
    <form method="POST" action="{{ route('inventory.stock-in.store') }}" class="glass-panel p-6 rounded-xl border border-gray-200 dark:border-erp-border space-y-6">
        @csrf
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">No. Transaksi *</label>
                <input type="text" name="number" value="STIN-202608-{{ rand(1000, 9999) }}" required class="w-full bg-gray-50 dark:bg-erp-dark border border-gray-200 dark:border-erp-border rounded-lg px-3 py-2 text-xs text-gray-900 dark:text-white">
            </div>
            <div>
        ...
        <div>
            ...
            <div class="text-[10px] text-gray-600 dark:text-gray-400 mt-1">Belum ada Gudang? <a href="{{ route('inventory.warehouses.index') }}" class="text-erp-green hover:underline">Buat di sini</a></div>
            @endif
        </div>
        ...
        <div class="space-y-4 pt-4 border-t border-gray-200 dark:border-erp-border">
            ...
            <div class="grid grid-cols-12 gap-3 items-center">
                <div class="col-span-6">
                    ...
                    @if($items->isEmpty())
                    <div class="text-[10px] text-gray-600 dark:text-gray-400 mt-1">Belum ada Barang terdaftar? <a href="{{ route('inventory.items.index') }}" class="text-erp-green hover:underline">Buat di sini</a></div>
                    @endif
                </div>
        ...
        <div class="flex justify-end space-x-3 pt-4 border-t border-gray-200 dark:border-erp-border">
            <a href="{{ route('inventory.stock-in.index') }}" class="bg-gray-50 dark:bg-erp-dark border border-gray-200 dark:border-erp-border hover:bg-gray-800 text-gray-700 dark:text-gray-300 font-medium py-2 px-4 rounded-lg text-xs">Batal</a>
