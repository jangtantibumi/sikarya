...
@section('title', 'Detail Transfer - ' . $transfer->number)
@section('content')
<div class="max-w-4xl mx-auto">
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ $transfer->number }}</h1>
            <p class="text-xs text-erp-green">Dari: {{ optional($transfer->sourceWarehouse)->name }} &rarr; Ke: {{ optional($transfer->destinationWarehouse)->name }}</p>
        </div>
        <a href="{{ route('inventory.transfers.index') }}" class="text-xs text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:text-white flex items-center">
