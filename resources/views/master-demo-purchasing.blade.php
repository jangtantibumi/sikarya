...
<body class="antialiased min-h-screen flex flex-col relative pb-20">
    ...
    <header class="glass sticky top-0 z-40 px-6 py-4 flex items-center justify-between">
        <div class="flex items-center gap-4">
            <a href="{{ route('master-demo', ['company' => $company->id]) }}" class="text-slate-400 hover:text-white transition">
                <i data-lucide="arrow-left" class="w-5 h-5"></i>
            </a>
            <div>
                <p class="text-xs font-semibold tracking-wider text-slate-400 uppercase">Northstar OS &middot; Purchasing</p>
                <h1 class="text-xl font-bold gradient-text">{{ $company->name }}</h1>
            </div>
        </div>
        <div class="flex items-center gap-6">
            <div class="flex items-center gap-2">
                ...
                <div class="hidden md:block text-sm">
                    <p class="font-medium text-slate-200">{{ $currentUser->name }}</p>
                    <p class="text-xs text-slate-400 capitalize">{{ str_replace('_', ' ', $currentUser->role) }}</p>
                </div>
            </div>
            <form method="post" action="{{ route('master-demo.logout') }}">
                @csrf
    ...
    <main class="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div class="flex items-center gap-2 overflow-x-auto pb-2 sm:pb-0 scrollbar-hide">
                ...
                @foreach($companies as $c)
                    <a href="{{ route('master-demo.purchasing', ['company' => $c->id]) }}" 
                       class="px-4 py-1.5 rounded-full text-sm font-medium whitespace-nowrap transition-all {{ $c->id === $company->id ? 'bg-indigo-500/20 text-indigo-300 border border-indigo-500/30' : 'bg-slate-800/50 text-slate-400 border border-slate-700/50 hover:bg-slate-800 hover:text-slate-300' }}">
                        {{ $c->name }}
                    </a>
                @endforeach
            </div>
        ...
        <div class="glass rounded-2xl overflow-hidden">
            <div class="px-6 py-5 border-b border-white/5 flex items-center justify-between">
                ...
                <span class="px-2.5 py-1 rounded-md bg-slate-800 text-xs font-medium text-slate-300 border border-white/5">{{ $orders->count() }} Orders</span>
            </div>
                        ...
                        @forelse($orders as $o)
                            <tr class="hover:bg-white/[0.02] transition-colors">
                                <td class="px-6 py-4 font-medium text-slate-200">
                                    {{ $o->number }}
                                    <div class="text-xs text-slate-500 mt-0.5">{{ $o->lines->count() }} items</div>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    @endphp
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium border {{ $color }} capitalize">
                                ...
                                <td class="px-6 py-4 flex items-center justify-center gap-2">
                                    @if($o->status === 'draft')
                                        <button onclick="submitPO({{ $o->id }})" class="px-3 py-1.5 rounded bg-amber-500/20 text-amber-300 hover:bg-amber-500/30 transition text-xs font-medium border border-amber-500/30">Submit</button>
                                    @elseif($o->status === 'submitted')
                                        <button onclick="decidePO({{ $o->id }}, 'approved')" class="px-3 py-1.5 rounded bg-emerald-500/20 text-emerald-300 hover:bg-emerald-500/30 transition text-xs font-medium border border-emerald-500/30">Approve</button>
                                        <button onclick="decidePO({{ $o->id }}, 'rejected')" class="px-3 py-1.5 rounded bg-rose-500/20 text-rose-300 hover:bg-rose-500/30 transition text-xs font-medium border border-rose-500/30">Reject</button>
                                    @elseif(in_array($o->status, ['approved', 'partially_received']))
                                        <button onclick="openReceiptModal({{ $o->id }}, {{ $o->lines->toJson() }})" class="px-3 py-1.5 rounded bg-cyan-500/20 text-cyan-300 hover:bg-cyan-500/30 transition text-xs font-medium border border-cyan-500/30">Receive Goods</button>
                                    @else
        ...
        <div class="glass rounded-2xl overflow-hidden mt-8">
            <div class="px-6 py-5 border-b border-white/5 flex items-center justify-between">
                ...
                <span class="px-2.5 py-1 rounded-md bg-slate-800 text-xs font-medium text-slate-300 border border-white/5">{{ $receipts->count() }} Receipts</span>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm whitespace-nowrap">
                    <thead>
                    ...
                    <tbody class="divide-y divide-white/5">
                        @forelse($receipts as $r)
                            <tr class="hover:bg-white/[0.02] transition-colors">
                                ...
                                <td class="px-6 py-4 text-slate-400">
                                    {{ \Carbon\Carbon::parse($r->received_date)->format('d M Y') }}
                                </td>
                                <td class="px-6 py-4 text-slate-300">
                                    {{ $r->receivedBy->name ?? '-' }}
                                </td>
