        ...
        @foreach($activeFeatures as $groupName => $groupFeatures)
            ...
            @foreach($groupFeatures as $f)
                ...
                @if($f['key'] === 'inventory' && !$isComingSoon)
                    <a class="nav-item" data-target="inventory_umkm" id="sidebar-nav-item-{{ $f['key'] }}" onclick="switchView('inventory_umkm')">
        ...
        <section id="view-core_security" class="view-section">
            ...
            <div class="card" style="margin-bottom: 16px;">
                ...
                <div class="list-item" style="cursor:pointer;" onclick="const d = document.getElementById('role-users-{{ $r->key }}'); if(d) { d.style.display = (d.style.display === 'none' || d.style.display === '') ? 'block' : 'none'; }">
                    ...
                    <div>
                        <span style="background: rgba(12, 53, 39,0.1); color: var(--success); padding: 4px 10px; border-radius: 99px; font-size: 11px; font-weight: bold;">{{ count($r->users) }} Users</span>
...
<section id="view-accounting" class="view-section">
    ...
    <div id="manual-journal-modal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); z-index: 1000; align-items: center; justify-content: center;">
            ...
            <div class="grid-2">
                ...
                <div style="display: flex; flex-direction: column; gap: 24px;">
                    ...
                    <div class="card" style="padding: 24px; border: 1px solid var(--panel-border); border-radius: 16px; box-shadow: 0 4px 20px rgba(0,0,0,0.05);">
                        ...
                        <form id="form-shift" method="POST" action="{{ route('master-demo.shifts.store') }}">
                            @csrf
                        ...
                        <div style="margin-top: 16px;">
                            @forelse(\App\Models\Shift::where('company_id', $company->id)->get() as $shift)
                                <div style="display: flex; justify-content: space-between; align-items: center; background: var(--panel-secondary); padding: 12px 16px; border-radius: 12px; margin-bottom: 10px; border: 1px solid var(--panel-border);">
                                    <div>
                                        <span style="font-weight: 600; color: var(--text-heading); font-size: 14px;">{{ $shift->name }}</span>
                                        <span style="color: var(--text-muted); font-size: 12px; margin-left: 10px; background: rgba(0,0,0,0.05); padding: 4px 8px; border-radius: 20px;">{{ substr($shift->start_time, 0, 5) }} - {{ substr($shift->end_time, 0, 5) }}</span>
                                    </div>
                                    <div style="display: flex; gap: 6px;">
                                        <button type="button" onclick="editShift('{{ $shift->id }}', '{{ $shift->name }}', '{{ substr($shift->start_time, 0, 5) }}', '{{ substr($shift->end_time, 0, 5) }}')" class="ios-btn" style="padding: 6px 12px; font-size: 11px; background: rgba(59,130,246,0.1); color: var(--text-accent); border-radius: 20px;"><i class="fa-solid fa-pencil" style="margin-right: 4px;"></i> Edit</button>
                                        <form method="POST" action="/master-demo/shifts/{{ $shift->id }}" onsubmit="return confirm('Hapus shift ini?');" style="margin: 0;">
                                            @csrf @method('DELETE')
                    ...
                    <div class="card" style="padding: 24px; border: 1px solid var(--panel-border); border-radius: 16px; margin-top: 24px;">
                        ...
                        <form method="POST" action="{{ route('master-demo.holidays.store') }}">
                            @csrf
                            <div style="display: flex; gap: 8px; margin-bottom: 12px;">
                        ...
                        <div style="margin-top: 16px;">
                            @forelse(\App\Models\Holiday::where('company_id', $company->id)->orderBy('start_date', 'asc')->get() as $holiday)
                                <div style="display: flex; justify-content: space-between; align-items: center; background: var(--panel-secondary); padding: 12px 16px; border-radius: 12px; margin-bottom: 10px; border: 1px solid var(--panel-border);">
                                    <div>
                                        <span style="font-weight: 600; color: var(--text-heading); font-size: 14px;">{{ $holiday->name }}</span>
                                    ...
                                    <div>
                                        <form method="POST" action="/master-demo/holidays/{{ $holiday->id }}" onsubmit="return confirm('Hapus hari libur ini?');" style="margin: 0;">
                                            @csrf @method('DELETE')
                                            <button type="submit" class="ios-btn" style="padding: 6px 12px; font-size: 11px; background: rgba(239,68,68,0.1); color: #ef4444; border-radius: 20px;"><i class="fa-solid fa-trash" style="margin-right: 4px;"></i> Hapus</button>
                                        </form>
                                    </div>
                        ...
                        <tbody>
                            ...
                            <tr class="hris-emp-row" id="emp-row-{{ $u->id }}" style="border-bottom: 1px solid var(--panel-border);" data-name="{{ strtolower($u->name) }}" data-role="{{ strtolower($u->job_title ?? '') }}">
                                ...
                                <td style="padding: 16px; text-align: right;">
                                    <button class="ios-btn ios-btn-secondary" style="padding: 6px 12px; font-size: 12px; margin-right: 8px;" onclick="openEditEmployeeModal('{{ $u->id }}', '{{ addslashes($u->name) }}', '{{ $u->email }}', '{{ (int)$u->base_salary }}', '{{ addslashes($u->job_title) }}', '{{ $u->employment_type }}', '{{ $u->role }}', '{{ $u->employee_code }}')"><i class="fa-solid fa-pen"></i> Edit</button>
                                ...
                                <ul>
                                    ...
                                    <li>
                                        <div class="org-card" style="border-color: rgba(59,130,246,0.3);">
                                            <div class="org-avatar" style="background: #0C3527;">{{ $staff->getInitials() }}</div>
        ...
        <section id="view-location_tracking" class="view-section">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 24px;">
                ...
                <div class="card" style="padding: 24px; border: 1px solid var(--panel-border); border-radius: 16px; box-shadow: 0 4px 20px rgba(0,0,0,0.05);">
                    ...
                    <form id="form-rest" method="POST" action="{{ route('master-demo.attendance-settings.store') }}" style="margin-bottom: 20px;">
                        @csrf
                    ...
                    <table class="data-table" style="width: 100%; font-size: 13px; border-collapse: separate; border-spacing: 0 8px;">
                        ...
                        <tbody>
                            @forelse($attendanceSettings ?? [] as $setting)
                            <tr style="background: var(--panel-secondary); box-shadow: 0 2px 8px rgba(0,0,0,0.02);">
                                <td style="padding: 12px; border-radius: 12px 0 0 12px; font-weight: 500;">{{ $setting->name }}</td>
                                <td style="padding: 12px;">{{ \Carbon\Carbon::parse($setting->rest_start_time)->format('H:i') }} - {{ \Carbon\Carbon::parse($setting->rest_end_time)->format('H:i') }}</td>
                                <td style="padding: 12px; border-radius: 0 12px 12px 0; text-align: right;">
                                    <div style="display: flex; gap: 6px; justify-content: flex-end;">
                                        <button type="button" class="ios-btn ios-btn-secondary" style="padding: 6px 12px; font-size: 12px;" onclick="editRest('{{ $setting->id }}', '{{ $setting->name }}', '{{ \Carbon\Carbon::parse($setting->rest_start_time)->format('H:i') }}', '{{ \Carbon\Carbon::parse($setting->rest_end_time)->format('H:i') }}')">
                                            <i class="fa-solid fa-edit"></i>
                                        </button>
                                        <form method="POST" action="/master-demo/attendance-settings/{{ $setting->id }}" onsubmit="return confirm('Hapus waktu istirahat ini?');" style="margin: 0;">
                                            @csrf
                                            @method('DELETE')
                ...
                <div class="card" style="padding: 24px; border: 1px solid var(--panel-border); border-radius: 16px; box-shadow: 0 4px 20px rgba(0,0,0,0.05);">
                    ...
                    <form id="form-overtime" method="POST" action="{{ route('master-demo.overtime.store') }}" style="margin-bottom: 20px;">
                        @csrf
                    ...
                    <table class="data-table" style="width: 100%; font-size: 13px; border-collapse: separate; border-spacing: 0 8px;">
                        ...
                        <tbody>
                            ...
                            <tr style="background: var(--panel-secondary); box-shadow: 0 2px 8px rgba(0,0,0,0.02);">
                                <td style="padding: 12px; border-radius: 12px 0 0 12px; font-weight: 500;">{{ $ot->name }}</td>
                                <td style="padding: 12px;">Rp {{ number_format($ot->rate_per_hour, 0, ',', '.') }}</td>
                                <td style="padding: 12px; border-radius: 0 12px 12px 0; text-align: right;">
                                    <div style="display: flex; gap: 6px; justify-content: flex-end;">
                                        <button type="button" onclick="editOT('{{ $ot->id }}', '{{ $ot->name }}', '{{ $ot->rate_per_hour }}')" class="ios-btn" style="padding: 6px 12px; font-size: 11px; background: rgba(59,130,246,0.1); color: var(--text-accent); border-radius: 20px;"><i class="fa-solid fa-pencil"></i></button>
                                        <form method="POST" action="/master-demo/overtime/{{ $ot->id }}" onsubmit="return confirm('Hapus jenis lembur ini?');" style="margin: 0;">
                                            @csrf @method('DELETE')
                            ...
                            <tr>
                                ...
                                <td>
                                    @if($att->location_coordinates)
                                        <a href="https://maps.google.com/?q={{ $att->location_coordinates }}" target="_blank" style="color: var(--accent);"><i class="fa-solid fa-map-marker-alt" style="margin-right: 4px;"></i> {{ $att->location_name ?? 'Cek Map' }}</a>
                                    @else
        ...
        <section id="view-chat_internal" class="view-section">
            <div class="chat-layout {{ (auth()->check() && (auth()->user()->isCEO() || auth()->user()->isManager() || auth()->user()->isPlatformAdmin())) ? 'has-announcements' : '' }}">
                <!-- Sidebar Kontak -->
                <div class="chat-channels">
                    ...
                    <div id="chat-channels-list">
                        ...
                        @if(auth()->user()->isCEO() || auth()->user()->isManager() || auth()->user()->isPlatformAdmin())
                ...
                @if(auth()->check() && (auth()->user()->isCEO() || auth()->user()->isManager() || auth()->user()->isPlatformAdmin()))
                    ...
                    <div style="flex: 1; overflow-y: auto; padding: 12px; display: flex; flex-direction: column; gap: 12px;">
                        ...
                        @forelse($activeAnnouncements as $ann)
                            <div style="padding: 12px; border: 1px solid var(--panel-border); border-radius: 8px; background: var(--bg-main); display: flex; flex-direction: column;">
                                <h4 style="margin: 0 0 4px 0; font-size: 13px; color: var(--text-main);">{{ $ann->title }} <span style="font-size: 9px; padding: 2px 6px; border-radius: 4px; background: rgba(12, 53, 39, 0.1); color: var(--primary); margin-left: 6px;">{{ strtoupper($ann->target_type) }}</span></h4>
