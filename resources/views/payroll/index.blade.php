    ...
    <div id="payroll-tab-components" class="payroll-tab-content" style="display: none;">
        <div style="display: flex; gap: 24px; align-items: flex-start;">
                    ...
                    <tbody>
                        ...
                        <tr style="background: var(--panel-secondary);">
                            <td style="padding: 14px 16px; border-radius: 12px 0 0 12px; font-weight: 600; color: var(--text-heading);">
                                {{ $comp->code }}<br>
                                <span style="font-size: 12px; font-weight: 500; color: var(--text-muted);">{{ $comp->name }}</span>
                            </td>
                            ...
                            <td style="padding: 14px 16px; font-weight: 600;">Rp {{ number_format($comp->default_amount, 0, ',', '.') }}</td>
                            <td style="padding: 14px 16px; border-radius: 0 12px 12px 0; text-align: right;">
                                <div style="display: flex; gap: 6px; justify-content: flex-end;">
                                    <button class="ios-btn-icon" onclick="editSalaryComponent('{{ $comp->id }}', '{{ $comp->code }}', '{{ $comp->name }}', '{{ $comp->type }}', '{{ $comp->is_default }}', '{{ $comp->default_amount }}')"><i class="fa-solid fa-pen"></i></button>
                                    <form method="POST" action="/master-demo/salary-components/{{ $comp->id }}" onsubmit="return confirm('Hapus komponen ini?');" style="margin: 0;">
                                        @csrf @method('DELETE')
            ...
            <div style="flex: 1; padding: 24px; background: rgba(255,255,255,0.8); backdrop-filter: blur(12px); border: 1px solid var(--panel-border); border-radius: 20px; box-shadow: 0 8px 32px rgba(0,0,0,0.08);">
                ...
                <form id="salcomp-form" method="POST" action="{{ route('master-demo.salary-components.store') }}">
                    @csrf
