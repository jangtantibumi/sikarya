...
<head>
    ...
    <title>Customer Portal - {{ $customer->name }}</title>
...
<div style="max-width: 600px; margin: 0 auto; background: var(--erp-bg); min-height: 100vh;">
    ...
    <div class="portal-header">
        <div>
            ...
            <h5 style="font-weight: 800; margin: 0; color: #1e293b; font-size: 20px;">{{ explode(' ', $customer->name)[0] }}</h5>
        </div>
    ...
    <div style="padding: 0 20px; margin-top: 16px;">
        ...
        @if($errors->any())
            <div class="alert" style="background: rgba(220, 38, 38, 0.1); color: var(--erp-danger); border: none; border-radius: 12px; font-weight: 500; font-size: 14px; padding: 12px 16px;">
                <i class="ph-fill ph-warning-circle" style="margin-right: 6px;"></i> {{ $errors->first() }}
            </div>
        @endif
    </div>

    <!-- Member Card -->
    <a href="{{ route('portal.card') }}" style="text-decoration: none; color: inherit; display: block;">
        <div class="member-card">
            <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                <div>
                    ...
                    <div class="tier-badge mt-2 mb-2">
                        <i class="ph-fill ph-crown"></i> {{ $customer->membership_level }}
                    </div>
                </div>
                <div style="text-align: right;">
                    <div class="qr-placeholder">
                        <img src="https://api.qrserver.com/v1/create-qr-code/?size=80x80&data={{ $customer->customer_code }}" alt="QR Code">
                    </div>
                    <div style="font-size: 11px; font-weight: 700; margin-top: 8px; letter-spacing: 1px; color: rgba(255,255,255,0.8);">{{ $customer->customer_code }}</div>
                </div>
            </div>
            <div style="margin-top: 16px;">
                ...
                <div class="points-display">{{ number_format($customer->total_points, 0, ',', '.') }}</div>
    ...
    <div style="padding: 0 20px;">
        ...
        @forelse($customer->reservations as $res)
            <div class="history-item">
                <div>
                    <div style="font-weight: 700; font-size: 15px; color: #1e293b; margin-bottom: 4px;">{{ $res->reservation_date->format('d/m/Y') }}</div>
                    <div style="font-size: 13px; color: #64748b; font-weight: 500; display: flex; align-items: center; gap: 8px;">
                        <i class="ph ph-clock"></i> {{ date('H:i', strtotime($res->reservation_time)) }} &bull; <i class="ph ph-users"></i> {{ $res->pax }} Orang
                    </div>
                </div>
                <div><span class="status-badge {{ strtolower($res->status) }}">{{ $res->status }}</span></div>
            </div>
        @empty
    ...
    <div class="bottom-nav">
        <a href="{{ route('portal.dashboard') }}" class="bottom-nav-item active">
            <i class="ph-fill ph-house"></i>
            <span>Home</span>
        </a>
        <a href="{{ route('portal.vouchers') }}" class="bottom-nav-item">
            <i class="ph-fill ph-ticket"></i>
            <span>Voucher</span>
        </a>
        <a href="{{ route('portal.loyalty') }}" class="bottom-nav-item">
            <i class="ph-fill ph-coins"></i>
            <span>Poin</span>
        </a>
        <a href="{{ route('portal.invoices') }}" class="bottom-nav-item">
            <i class="ph-fill ph-receipt"></i>
            <span>Transaksi</span>
        </a>
        <a href="{{ route('portal.profile') }}" class="bottom-nav-item">
...
<div class="modal fade" id="modalReserve" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <form action="{{ route('portal.reserve') }}" method="POST">
                @csrf
...
<div class="modal fade" id="modalFeedback" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <form action="{{ route('portal.feedback') }}" method="POST">
                @csrf
                <div class="modal-body">
