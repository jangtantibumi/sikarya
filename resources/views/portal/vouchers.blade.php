...
<div style="max-width: 600px; margin: 0 auto; background: var(--erp-bg); min-height: 100vh;">
    ...
    <div style="padding: 16px 20px;">
        <div style="background: var(--erp-primary); color: white; border-radius: 16px; padding: 16px 20px; margin-bottom: 24px; display: flex; justify-content: space-between; align-items: center;">
            <div>
                ...
                <div style="font-size: 24px; font-weight: 800;">{{ number_format($customer->total_points) }} pts</div>
            </div>
            <i class="ph-fill ph-coins" style="font-size: 36px; color: #f59e0b;"></i>
        </div>

        @if(session('success'))
            <div class="alert alert-success" style="border-radius: 12px; font-size: 14px; font-weight: 600;">{{ session('success') }}</div>
        @endif
        @if($errors->any())
            <div class="alert alert-danger" style="border-radius: 12px; font-size: 14px; font-weight: 600;">{{ $errors->first() }}</div>
        @endif
        ...
        @forelse($availableVouchers as $v)
            <div class="voucher-card">
                <div>
                    <div style="font-weight: 700; font-size: 15px; color: #1e293b;">{{ $v->name }}</div>
                    <div style="font-size: 12px; color: #64748b; margin: 4px 0 8px;">{{ $v->description ?? 'Diskon potongan pembelian F&B' }}</div>
                    <div style="font-size: 12px; font-weight: 700; color: var(--text-accent);">Biaya: 100 Pts</div>
                </div>
                <form action="{{ route('portal.vouchers.redeem', $v->id) }}" method="POST">
                    @csrf
        ...
        @forelse($myVouchers as $mv)
            <div class="voucher-card" style="background: rgba(12, 53, 39, 0.05); border: 1px dashed #0C3527;">
                <div>
                    <div style="font-weight: 700; font-size: 14px; color: #1e293b;">{{ $mv->voucher->name ?? 'Voucher Diskon' }}</div>
                    <div style="font-size: 11px; color: #64748b; margin-top: 2px;">Ditukarkan pada: {{ $mv->redeemed_at ? $mv->redeemed_at->format('d/m/Y H:i') : '-' }}</div>
                </div>
    ...
    <div class="bottom-nav">
        ...
        <a href="{{ route('portal.vouchers') }}" class="bottom-nav-item active"><i class="ph-fill ph-ticket"></i><span>Voucher</span></a>
