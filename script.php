...
// 2. Update card actions
$oldActions = '            <div class="org-card-actions" onclick="event.stopPropagation()">
                <button class="org-btn-action" onclick="openActionModal(\'${user.id}\', \'edit\')"><i class="fa-solid fa-pen"></i> Edit</button>
                <button class="org-btn-action" onclick="openActionModal(\'${user.id}\', \'review\')"><i class="fa-solid fa-chart-pie"></i> Review</button>
                <button class="org-btn-action" onclick="openActionModal(\'${user.id}\', \'task\')"><i class="fa-solid fa-tasks"></i> Assign</button>
            </div>';
$newActions = '            <div class="org-card-actions" onclick="event.stopPropagation()">
...
// 3. Add Custom Drawers and Modals
$modalsHtml = <<<HTML
...
$content = str_replace('<!-- Action Modals -->', $modalsHtml . "\n    <!-- Action Modals -->", $content);
...
$jsFunctions = <<<JS
    ...
    function openDeleteModal(id, name, pos, dept) {
        ...
        document.getElementById('del-emp-pos').innerText = `\${pos} • \${dept}`;
...
$content = str_replace('// Drawer Logic', $jsFunctions . "\n    // Drawer Logic", $content);
...
$content = str_replace('/* Org Drawer Styling */', $cssTabs . "\n    /* Org Drawer Styling */", $content);

// Remove old submitOrgAction
$content = preg_replace('/async function submitOrgAction.*?\}\s*\} /s', '', $content);
$content = preg_replace('/function openActionModal.*?\n\s+\}/s', '', $content);
$content = str_replace("alert(err.error || 'Action failed.');", "showToastMsg(err.error || 'Action failed.', 'error');", $content);
$content = str_replace("alert('Network error. Check console.');", "showToastMsg('Network error. Check console.', 'error');", $content);
$content = str_replace("alert('Action successful!');", "showToastMsg('Action successful!', 'success');", $content);
