    ...
    <div id="manual-journal-modal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); z-index: 1000; align-items: center; justify-content: center;">
        ...
        <section id="view-recipes" class="view-section">
            <div class="card" style="margin-bottom: 24px;">
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(in_array(Auth::user()->role, ['superadmin', 'manager', 'mgr_production'])): ?>
                ...
                <div style="background: rgba(0,0,0,0.2); padding: 16px; border-radius: 8px; margin-bottom: 24px; border: 1px solid var(--panel-border);">
                    ...
                    <form method="post" action="<?php echo e(route('master-demo.recipes.store')); ?>">
                        ...
                        <div class="grid-2">
                            <div>
                                <label style="display: block; margin-bottom: 8px; font-size: 12px; color: var(--text-muted);">Nama Barang Jadi</label>
                                <select name="product_id" class="form-control" style="background: rgba(255,255,255,0.05); border: 1px solid var(--panel-border); color: white; padding: 10px; border-radius: 8px; width: 100%; margin-bottom: 16px;" required>
                                    <option value="">-- Pilih Produk Master --</option>
                                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = \App\Models\Product::where('company_id', $company->id)->get(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                                        <option value="<?php echo e($p->id); ?>"><?php echo e($p->name); ?></option>
                                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::endLoop(); ?><?php endif; ?><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::closeLoop(); ?><?php endif; ?>
                                </select>
                                <label style="display: block; margin-bottom: 8px; font-size: 12px; color: var(--text-muted);">Nama Resep/SKU</label>
                                <input type="text" name="name" class="form-control" placeholder="Misal: Roti Manis Coklat Premium" style="background: rgba(255,255,255,0.05); border: 1px solid var(--panel-border); color: white; padding: 10px; border-radius: 8px; width: 100%; margin-bottom: 16px;" required>
                                <label style="display: block; margin-bottom: 8px; font-size: 12px; color: var(--text-muted);">Yield (Hasil per 1x Resep)</label>
                                <div style="display: flex; gap: 8px; margin-bottom: 16px;">
                                    <input type="number" name="yield_quantity" value="1" class="form-control" style="background: rgba(255,255,255,0.05); border: 1px solid var(--panel-border); color: white; padding: 10px; border-radius: 8px; width: 100%;" required>
                                    <input type="text" name="yield_unit" value="Pcs" class="form-control" style="background: rgba(255,255,255,0.05); border: 1px solid var(--panel-border); color: white; padding: 10px; border-radius: 8px; width: 80px;" required>
                                </div>
                            </div>
                            <div>
                                ...
                                <div id="recipe-items-container">
                                    <div class="recipe-item-row" style="display: flex; gap: 8px; margin-bottom: 8px;">
                                        <select name="materials[]" class="form-control material-select" onchange="calculateRecipeCost()" style="background: rgba(255,255,255,0.05); border: 1px solid var(--panel-border); color: white; padding: 8px 12px; border-radius: 6px; flex: 2;" required>
                                        ...
                                        <input type="number" step="0.01" name="quantities[]" class="form-control material-qty" oninput="calculateRecipeCost()" placeholder="Gram" style="background: rgba(255,255,255,0.05); border: 1px solid var(--panel-border); color: white; padding: 8px 12px; border-radius: 6px; flex: 1;" required>
                                    </div>
                                </div>
                ...
                <?php else: ?>
                <div style="background: rgba(0,0,0,0.2); padding: 40px; border-radius: 8px; border: 1px solid var(--panel-border); text-align: center;">
                    <i class="fa-solid fa-lock" style="font-size: 48px; color: var(--text-muted); margin-bottom: 16px; opacity: 0.5;"></i>
                    <h4 style="margin: 0 0 8px 0; font-size: 18px; color: var(--danger);">Akses Terbatas</h4>
                    <p style="font-size: 14px; color: var(--text-muted);">Hanya CEO & Manajer Produksi yang memiliki wewenang mengelola Master Resep (BOM).</p>
                </div>
                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
            </div>
        </section>
            ...
            <div class="card" style="margin-bottom: 24px;">
                ...
                <div class="grid-2">
                    ...
                    <div>
                        ...
                        <h4 style="margin: 24px 0 12px 0; font-size: 14px;">Daftar Produksi Selesai (Batch)</h4>
                        <div style="background: rgba(0,0,0,0.2); padding: 16px; border-radius: 8px; border: 1px solid var(--panel-border); max-height: 300px; overflow-y: auto;">
                            <table style="width: 100%; border-collapse: collapse; font-size: 12px;">
                                <thead>
                                    <tr style="border-bottom: 1px solid var(--panel-border); color: var(--text-muted); text-align: left;">
                                        <th style="padding: 8px;">Tgl</th>
                                        <th style="padding: 8px;">No. Batch</th>
                                        <th style="padding: 8px;">Barang Jadi</th>
                                        <th style="padding: 8px;">Qty Selesai</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = \App\Models\ProductionOrder::with('product')->where('company_id', $company->id)->where('status', 'completed')->orderBy('created_at', 'desc')->take(10)->get(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $po): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                                        <tr style="border-bottom: 1px solid rgba(255,255,255,0.05);">
                                            <td style="padding: 8px; color: var(--text-muted);"><?php echo e($po->created_at->format('d/m H:i')); ?></td>
                                            <td style="padding: 8px;"><?php echo e($po->number); ?></td>
                                            <td style="padding: 8px;"><?php echo e($po->product ? $po->product->name : '-'); ?></td>
                                            <td style="padding: 8px; font-weight: bold; color: var(--success);"><?php echo e($po->completed_quantity + 0); ?></td>
                                        </tr>
                                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::endLoop(); ?><?php endif; ?><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::closeLoop(); ?><?php endif; ?>
                                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(\App\Models\ProductionOrder::where('company_id', $company->id)->where('status', 'completed')->count() == 0): ?>
                                        <tr><td colspan="4" style="text-align: center; padding: 16px; color: var(--text-muted);">Belum ada produksi selesai hari ini.</td></tr>
                                    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                </tbody>
                            </table>
                        </div>
