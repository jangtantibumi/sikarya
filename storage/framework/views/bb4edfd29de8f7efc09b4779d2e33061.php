<?php $__env->startSection('title', 'Adjustment Baru'); ?>
<?php $__env->startSection('content'); ?>
<div class="max-w-4xl mx-auto">
    <h1 class="text-2xl font-bold text-gray-900 dark:text-white mb-6">Input Stock Adjustment Baru</h1>
    <form method="POST" action="<?php echo e(route('inventory.adjustments.store')); ?>" class="glass-panel p-6 rounded-xl border border-gray-200 dark:border-erp-border space-y-6">
        <?php echo csrf_field(); ?>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">No. Adjustment *</label>
                <input type="text" name="number" value="ADJ-202608-<?php echo e(rand(1000, 9999)); ?>" required class="w-full bg-gray-50 dark:bg-erp-dark border border-gray-200 dark:border-erp-border rounded-lg px-3 py-2 text-xs text-gray-900 dark:text-white">
            </div>
            <div>
            ...
            <div>
                ...
                <select name="type" required class="w-full bg-gray-50 dark:bg-erp-dark border border-gray-200 dark:border-erp-border rounded-lg px-3 py-2 text-xs text-gray-900 dark:text-white">
                    <option value="addition">Addition (Penambahan Stok)</option>
                    <option value="reduction">Reduction (Pengurangan Stok)</option>
                </select>
            </div>
        </div>
        ...
        <div class="space-y-4 pt-4 border-t border-gray-200 dark:border-erp-border">
            <h3 class="text-xs font-bold text-erp-green uppercase tracking-wider">Detail Item Adjustment</h3>
            <div class="grid grid-cols-12 gap-3 items-center">
                ...
                <div class="col-span-3">
                    <label class="block text-[11px] text-gray-600 dark:text-gray-400">Adjustment Qty (+/-)</label>
                    <input type="number" name="items[0][adjustment_qty]" value="-5" class="w-full bg-gray-50 dark:bg-erp-dark border border-gray-200 dark:border-erp-border rounded-lg px-3 py-1.5 text-xs text-gray-900 dark:text-white">
                </div>
                <div class="col-span-3">
                    <label class="block text-[11px] text-gray-600 dark:text-gray-400">Alasan</label>
                    <input type="text" name="items[0][reason]" value="Barang rusak saat bongkar" class="w-full bg-gray-50 dark:bg-erp-dark border border-gray-200 dark:border-erp-border rounded-lg px-3 py-1.5 text-xs text-gray-900 dark:text-white">
                </div>
            </div>
        </div>
        ...
        <div class="flex justify-end space-x-3 pt-4 border-t border-gray-200 dark:border-erp-border">
            <button type="submit" class="bg-erp-green hover:bg-emerald-400 text-gray-900 font-bold py-2 px-6 rounded-lg text-xs">Simpan Draft Adjustment</button>
        </div>
    </form>
</div>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('inventory.layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\suba-erp-master-local-latest\resources\views\inventory\adjustments\create.blade.php ENDPATH**/ ?>